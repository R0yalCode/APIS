...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(f"Username: {username}, Password: {password}")  # Línea de depuración
            user = authenticate(request, username=username, password=password)
            if user is not None:
                login(request, user)
                return redirect('user')  # Cambia la URL según tu necesidad
            else:
                print("Invalid username or password")  # Línea de depuración
                form.add_error(None, 'Invalid username or password')
        else:
            print("Form is not valid")  # Línea de depuración
    else:
